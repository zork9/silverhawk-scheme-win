  ...
  (let ((port (command-output)))
    ...
    (if info
        (begin (write-char #\space port)
               (display info port)))
    (newline port)
    (write-line "See http://gomez.yellowcouch.org/silverhawk.software/ for more information."
		port)
    (write-line "Please report bugs to USENIX@yellowcouch.org."
                port)
    (if (not (batch-mode?))
